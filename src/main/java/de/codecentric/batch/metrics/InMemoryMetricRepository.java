 ...
 * @author Dave Syer
 */
public class InMemoryMetricRepository implements MetricRepository, MultiMetricRepository, PrefixMetricReader {
	...
	public void increment(Delta<?> delta) {
		...
		final Number amount = delta.getValue();
		...
		this.metrics.update(metricName, new Callback<Metric<?>>() {
			...
			public Metric<?> modify(Metric<?> current) {
				if (current != null) {
					...
					if (metric.getValue() instanceof Double && amount instanceof Double) {
						return new Metric<Double>(metricName, metric.getValue().doubleValue() + amount.doubleValue(), timestamp);
					} else {
						return new Metric<Long>(metricName, metric.increment(amount.intValue()).getValue(), timestamp);
					}
				} else {
					return new Metric<Number>(metricName, amount, timestamp);
				}
			}
		});
	}
